...
export class MultiSiteComponent {
  ...
  loadLayout() {
    const layoutConfig = this.featureService.getLayoutWidgetsByFeature('production-tracking');
    const title = 'PRODUCTION TRACKING';
    const widget_apis = environment.PRODUCTION_TRACKING as Apis;
